...
namespace ast {
  ...
  struct printer : printer_base {
    ...
    template <typename T> void renderList( T const &_col ) const {
    ...
    template <typename T>
    void renderCollection( T const &_col, std::string const &_pre,
  ...
  struct ast_dumper {
    ...
    void operator()( ast::special const &_v );
    ...
    template <typename T> void operator()( T const &_v ) {
      assert( !"NOT HANDLED" );
    }
  };
}
